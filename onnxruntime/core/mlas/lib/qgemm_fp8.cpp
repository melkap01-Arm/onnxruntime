    ...
    )
{
    ...
    if (GetMlasPlatform().MlasFp8GemmPackBSizeOverride != nullptr) {
        return GetMlasPlatform().MlasFp8GemmPackBSizeOverride(N, K, BlockSizeK, BlockSizeN, Fp8Type);
    }
#else
    MLAS_UNREFERENCED_PARAMETER(N);
    MLAS_UNREFERENCED_PARAMETER(K);
    MLAS_UNREFERENCED_PARAMETER(BlockSizeK);
    MLAS_UNREFERENCED_PARAMETER(BlockSizeN);
    MLAS_UNREFERENCED_PARAMETER(Fp8Type);
#endif
    return 0;
}
...
{
#if defined(USE_KLEIDIAI)
    if (GetMlasPlatform().MlasFp8GemmPackBScaleSizeOverride != nullptr) {
        return GetMlasPlatform().MlasFp8GemmPackBScaleSizeOverride(N, K, BlockSizeK, BlockSizeN, Fp8Type);
    }
#else
    MLAS_UNREFERENCED_PARAMETER(N);
    MLAS_UNREFERENCED_PARAMETER(K);
...
{
#if defined(USE_KLEIDIAI)
    if (GetMlasPlatform().MlasFp8GemmPackBOverride != nullptr) {
        return GetMlasPlatform().MlasFp8GemmPackBOverride(
            N, K, B, ldb, ScaleB, BlockSizeK, BlockSizeN, Fp8Type, PackedB, PackedScaleB);
    }
#else
    MLAS_UNREFERENCED_PARAMETER(N);
    MLAS_UNREFERENCED_PARAMETER(K);
    ...
    MLAS_UNREFERENCED_PARAMETER(ScaleB);
    MLAS_UNREFERENCED_PARAMETER(BlockSizeK);
    MLAS_UNREFERENCED_PARAMETER(BlockSizeN);
    MLAS_UNREFERENCED_PARAMETER(Fp8Type);
    MLAS_UNREFERENCED_PARAMETER(PackedB);
    MLAS_UNREFERENCED_PARAMETER(PackedScaleB);
#endif
    return false;
}
...
{
#if defined(USE_KLEIDIAI)
    if (GetMlasPlatform().MlasFp8GemmPackedBIsSupportedOverride != nullptr) {
        return GetMlasPlatform().MlasFp8GemmPackedBIsSupportedOverride(Shape, DataParams, BatchN);
    }
#else
...
#if defined(USE_KLEIDIAI)
    if (GetMlasPlatform().MlasFp8GemmBatchOverride != nullptr &&
        GetMlasPlatform().MlasFp8GemmBatchOverride(Shape, DataParams, BatchN, ThreadPool)) {
        return;
    }
    for (size_t batch = 0; batch < BatchN; ++batch) {
        ORT_ENFORCE(DataParams[batch].AFloat == nullptr,
                    "FP8 GEMM original FP32 A requires a backend that can quantize while packing LHS.");
    }
#endif
