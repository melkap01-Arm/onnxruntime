...
void RunDynamicQuantMatMulFp8WithSharedPrepack(DynamicQuantMatMulFp8SessionTester& test,
  ...
  test.ExecuteModel(model,
                    session,
                    expected_failure == nullptr ? OpTester::ExpectResult::kExpectSuccess
                                                : OpTester::ExpectResult::kExpectFailure,
                    expected_failure == nullptr ? "" : expected_failure,
                    nullptr,
                    feeds,
                    output_names,
                    kCpuExecutionProvider);
  if (expected_failure == nullptr) {
...
TEST(DynamicQuantMatMulFp8, SharedPrepackedWeightsRejectMismatchedFp8Metadata) {
  constexpr int64_t M = 4;
  constexpr int64_t N = 4;
  constexpr int64_t K = 4;
  constexpr int64_t BlockSize = 2;
  constexpr size_t MetadataBufferIndex = 2;
  constexpr size_t Fp8ModeMetadataIndex = 5;
  ...
  DynamicQuantMatMulFp8SessionTester test_1("DynamicQuantMatMulFp8", 1, onnxruntime::kMSDomain);
  test_1.AddAttribute<int64_t>("fp8_type", Fp8TensorProtoType<Float8E4M3FN>::value);
  test_1.AddAttribute<int64_t>("block_size_k", BlockSize);
  test_1.AddAttribute<int64_t>("block_size_n", BlockSize);
  test_1.AddInput<float>("A", {M, K}, a_data);
  test_1.AddInput<float>("B", {K, N}, b_data, true /*initializer*/);
  ...
  auto& packed_weights = prepacked_weights_container.prepacked_weights_map_.begin()->second;
  ASSERT_GT(packed_weights.buffers_.size(), MetadataBufferIndex);
  ASSERT_GE(packed_weights.buffer_sizes_[MetadataBufferIndex],
            (Fp8ModeMetadataIndex + 1) * sizeof(int64_t));
  auto* metadata = static_cast<int64_t*>(packed_weights.buffers_[MetadataBufferIndex].get());
  ASSERT_NE(metadata, nullptr);
  metadata[Fp8ModeMetadataIndex] = static_cast<int64_t>(MLAS_FP8_MODE_E5M2_INF);

  DynamicQuantMatMulFp8SessionTester test_2("DynamicQuantMatMulFp8", 1, onnxruntime::kMSDomain);
  test_2.AddAttribute<int64_t>("fp8_type", Fp8TensorProtoType<Float8E4M3FN>::value);
  test_2.AddAttribute<int64_t>("block_size_k", BlockSize);
  test_2.AddAttribute<int64_t>("block_size_n", BlockSize);
  test_2.AddInput<float>("A", {M, K}, a_data);
  test_2.AddInput<float>("B", {K, N}, b_data, true /*initializer*/);
  ...
  RunDynamicQuantMatMulFp8WithSharedPrepack(
      test_2, b, prepacked_weights_container, shared_prepack_count,
